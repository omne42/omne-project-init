...
fn render_string(value: &str, config: &InitConfig) -> String {
    value
        ...
        .replace("__REPO_CHECK_SCHEMA_VERSION__", REPO_CHECK_SCHEMA_VERSION)
...
fn cleanup_existing_scaffold(config: &InitConfig) -> Result<(), String> {
    ...
    if existing.schema_version != REPO_CHECK_SCHEMA_VERSION {
        return Err(format!(
            "target directory was generated by repo-check.toml schema version {}.\n\n`--force` only supports in-place regeneration from schema version {} so scaffold metadata stays compatible.",
            existing.schema_version, REPO_CHECK_SCHEMA_VERSION
        ));
    }
...
fn load_stored_scaffold_config(target_dir: &Path) -> Result<Option<StoredScaffoldConfig>, String> {
    ...
    let schema_version = values
        ...
        .unwrap_or(LEGACY_REPO_CHECK_SCHEMA_VERSION)
        .to_string();
    validate_repo_check_schema_version(&schema_version)?;
...
fn validate_repo_check_schema_version(schema_version: &str) -> Result<(), String> {
    if schema_version == REPO_CHECK_SCHEMA_VERSION {
        return Ok(());
    }
    Err(format!(
        "unsupported repo-check.toml schema_version `{schema_version}` (expected `{REPO_CHECK_SCHEMA_VERSION}`)"
    ))
}
...
mod tests {
    ...
    fn load_stored_scaffold_config_accepts_real_toml_with_tables() {
        ...
        assert_eq!(stored.schema_version, REPO_CHECK_SCHEMA_VERSION);
    ...
    #[test]
    fn load_stored_scaffold_config_rejects_unknown_schema_version() {
        let sandbox = TempDir::new("stored-config-schema-version");
        fs::write(
            sandbox.path().join("repo-check.toml"),
            concat!(
                "template_version = \"0.1.0\"\n",
        ...
        let error = load_stored_scaffold_config(sandbox.path())
            .expect_err("unknown schema version should fail");
        assert!(error.contains("unsupported repo-check.toml schema_version"));
    }

    #[test]
    fn load_stored_scaffold_config_accepts_legacy_config_without_schema_version() {
        let sandbox = TempDir::new("stored-config-legacy-schema");
        fs::write(
            sandbox.path().join("repo-check.toml"),
            concat!(
                "template_version = \"0.1.0\"\n",
        ...
        let stored = load_stored_scaffold_config(sandbox.path())
            .expect("load stored scaffold config")
            .expect("stored scaffold config should exist");
        assert_eq!(stored.schema_version, LEGACY_REPO_CHECK_SCHEMA_VERSION);
    }

    #[test]
    fn render_string_writes_repo_check_schema_version_placeholder() {
        let rendered = render_string(
            "schema_version = \"__REPO_CHECK_SCHEMA_VERSION__\"\n",
            &test_config(Path::new(".")),
        );
        assert_eq!(
            rendered,
            format!("schema_version = \"{REPO_CHECK_SCHEMA_VERSION}\"\n")
        );
    }
